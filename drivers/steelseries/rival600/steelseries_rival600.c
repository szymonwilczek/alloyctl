 ...
 * Maintainer: Dawid Prusinowski <prusdavid9@gmail.com>
 *
 * Configuration uses HID reports on USB interface 0.
 * Sensitivity, polling, buttons, and save use HID output reports.
 * Zone color and lighting effects use HID feature reports (HIDIOCSFEATURE).
 * Commands send fire-and-forget.
 */
#include <stdio.h>
#include <string.h>

#include "art_steelseries_rival600.h"
#include "hid.h"
#include "lib/mouse.h"
...
static const char *const r600_fx_names[] = {
	"STEADY", "BREATHE", "BREATHE FAST", "COLOR SHIFT", "DISCO",
};

struct r600_color_stop {
	uint8_t pos_percent;
	struct alloy_rgb color;
};

static size_t r600_pack_gradient(uint8_t led_id, uint16_t duration_ms,
				 uint8_t repeat,
				 const struct r600_color_stop *stops,
				 size_t num_stops, uint8_t *buf)
{
	size_t n = 0;
	size_t i;
	uint8_t last_real_pos = 0;

	buf[n++] = R600_CMD_COLOR; /* 0x05 */
	buf[n++] = 0x00;

	/* 28-byte header */
	memset(buf + n, 0, 28);
	buf[n + 0] = led_id; /* led_id offset 0 */
	buf[n + 5] = led_id; /* led_id offset 5 */
	buf[n + 6] = (uint8_t)(duration_ms & 0xFF);
	buf[n + 7] = (uint8_t)((duration_ms >> 8) & 0xFF);
	buf[n + 22] = repeat;
	buf[n + 23] = 0x00; /* triggers (continuous) */
	buf[n + 27] = (uint8_t)num_stops;
	n += 28;

	/* Initial color */
	buf[n++] = stops[0].color.r;
	buf[n++] = stops[0].color.g;
	buf[n++] = stops[0].color.b;

	/* Each color stop: R, G, B, delta_pos */
	for (i = 0; i < num_stops; i++) {
		uint8_t real_pos =
			(uint8_t)((stops[i].pos_percent * 255) / 100);
		uint8_t delta_pos = (uint8_t)(real_pos - last_real_pos);

		buf[n++] = stops[i].color.r;
		buf[n++] = stops[i].color.g;
		buf[n++] = stops[i].color.b;
		buf[n++] = delta_pos;
			     ...
			     uint8_t *buf)
{
	...
	switch (fx) {
	case 1: { /* BREATHE */
		const struct r600_color_stop stops[3] = {
			{ 0, color },
			{ 50, { 0x00, 0x00, 0x00 } },
			{ 100, color },
		};
		return r600_pack_gradient((uint8_t)zone, 3000, 0x00, stops, 3,
					  buf);
	}
	case 2: { /* BREATHE FAST */
		const struct r600_color_stop stops[3] = {
		...
		return r600_pack_gradient((uint8_t)zone, 1200, 0x00, stops, 3,
					  buf);
	}
	case 3: { /* COLOR SHIFT / RAINBOW */
		const struct r600_color_stop stops[4] = {
			{ 0, { 0xFF, 0x00, 0x00 } },
			{ 33, { 0x00, 0xFF, 0x00 } },
			{ 66, { 0x00, 0x00, 0xFF } },
			{ 100, { 0xFF, 0x00, 0x00 } },
		};
		return r600_pack_gradient((uint8_t)zone, 5000, 0x00, stops, 4,
					  buf);
	}
	case 4: { /* DISCO */
		const struct r600_color_stop stops[7] = {
			{ 0, { 0xFF, 0x00, 0x00 } },
			{ 16, { 0xFF, 0xFF, 0x00 } },
			{ 33, { 0x00, 0xFF, 0x00 } },
			{ 50, { 0x00, 0xFF, 0xFF } },
		...
		return r600_pack_gradient((uint8_t)zone, 3000, 0x00, stops, 7,
					  buf);
	}
	case 0: /* STEADY */
	default: {
		const struct r600_color_stop stops[1] = {
			{ 0, color },
		};
		return r600_pack_gradient((uint8_t)zone, 1000, 0x01, stops, 1,
					  buf);
	}
	}
}
			     ...
			     const struct alloy_config *cfg)
{
	...
	/* 8 LED zones (led_id 0x00 through 0x07) sent via HID Feature Reports */
	for (i = 0; i < 8; i++) {
		len = r600_build_zone_color(cfg, i, buf);
...
{
	uint8_t buf[R600_REPORT_SIZE];
	size_t len = r600_build_buttons(cfg, buf);
...
static void r600_defaults(const struct alloy_driver *drv,
			  struct alloy_config *cfg)
{
	struct alloy_mouse_config *m = alloy_mouse_cfg(cfg);

	alloy_mouse_defaults(drv, cfg);
	m->dpi_count = 2;
	m->dpi[0][0] = 800;
	m->dpi[0][1] = 800;
	m->dpi[1][0] = 1600;
...
static const struct alloy_devinfo r600_info = {
	...
	.fx_names = r600_fx_names,
	.num_fx = ALLOY_ARRAY_SIZE(r600_fx_names),
	.ext = &r600_mouse,
};
...
#define R600_DRIVER(sym, drv_name, pid)                           \
	static const struct alloy_driver sym = {                  \
		...
		.data = &r600_info,                               \
		.ascii_art = alloy_art_steelseries_rival600,      \
		.cli_tables = r600_cli,                           \
