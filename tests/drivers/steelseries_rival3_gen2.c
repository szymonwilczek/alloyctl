...
ALLOY_TEST(test_dpi_packet)
{
	...
	cfg.mouse.dpi[0][0] = 200;
	cfg.mouse.dpi[0][1] = 8500;
	len = r3g2_build_dpi(&cfg, buf);
	ASSERT_EQ(len, 5);
...
ALLOY_TEST(test_colors_packet)
{
	...
	r3g2()->config_defaults(r3g2(), &cfg);
	cfg.common.zone_color[0] = (struct alloy_rgb){ 0x11, 0x22, 0x33 };
	cfg.common.zone_color[1] = (struct alloy_rgb){ 0x44, 0x55, 0x66 };
	cfg.common.zone_color[2] = (struct alloy_rgb){ 0x77, 0x88, 0x99 };
...
ALLOY_TEST(test_brightness_packet)
{
	...
	cfg.common.brightness = 255; /* clamps to 100 */
